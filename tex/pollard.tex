...
\begin{figure}[H]
    ...
    \begin{minipage}{0.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{img/comparePollardVnolog.png}
        \caption{Sem Escala Logarítmica.}
        \label{fig:pollardvlog}
    \end{minipage}
    \hfill
    \begin{minipage}{0.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{img/comparePollardVlog.png}
        \caption{Escala Logarítmica.}
        \label{fig:pollardvnolog}
    \end{minipage}
\end{figure}
...
Podemos perceber pelos resultados e pela discussão acima que o algoritmo de Pollard $p-1$ é um excelente algoritmo em comparação a força bruta para quando temos muitos dígitos em $n$. E mais ainda, quando o fator primo $p$ de $n$ é tal que $p-1$ é $B-$suave. Assim, apesar de um bom método, ele se mostra relativamente limitado. Porém sobre certas condições como a suavidade de $p-1$, ele é altamente eficiente.
