...
podemos ficar bastante tentados em possuir um expoente público pequeno, 
...
do RSA se deve a fatoração de $n$, então diminuir o expoente parece
rasoável.
...
Suponhamos que um emissor quer enviar uma mensagem encriptada
...
o emissor se aperceba e colecionar os $k$ criptogramas.

Simplificando, considere que todos possuem $e_i = 3$. Podemos mostrar que se
$k \geq 3$, o intruso consegue recruperar a mensagem $M$ a partir de $C_1, C_2, C_3, \cdots$
...
poderiamos encontrar fatoração deles). E pelo Teorema do Resto Chinês (TRC),
...
pode-se recuperar $M$ assim que se tenha $k > e$ onde $k$ é o número de
...
É essencial, mesmo que facilite a encriptação, uso de expoentes relativamente
altos. Uma boa recomendação é o uso de no mínimo $2^{16}+1$, que de certa forma
facilita esses calculos e ainda é relativamente grande. 